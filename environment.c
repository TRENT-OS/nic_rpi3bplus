...
{
	TimeServer_sleep(TimeServer_PRECISION_USEC, nMicroSeconds);
}
...
int SetPowerStateOn (unsigned nDeviceId)
{
	...
	if (!MailboxInterface_getTag (PROPTAG_SET_POWER_STATE, &PowerState, sizeof PowerState, 8)
...
int GetMACAddress (unsigned char Buffer[6])
{
	...
	if(!MailboxInterface_getTag(PROPTAG_GET_MAC_ADDRESS, &MACAddress, sizeof(MACAddress), 0))
	{
