...
void Bcm54213Device (void)
{
	m_bInterruptConnected = FALSE;
	m_tx_cbs = 0;
	m_rx_cbs = 0;
	m_link = 0;
	SpinLock(&m_TxSpinLock,IRQ_LEVEL);
	// int ret = dma_manager();
	// if(ret)
	// {
	// 	pr_err("Allocation of dma_manager fails.");
	// 	exit(-1);
	// }
}

void _Bcm54213Device (void)
{
	intr_disable ();
	...
	dma_free(m_tx_cbs,DMA_ALIGNEMENT);
	dma_free(m_rx_cbs,DMA_ALIGNEMENT);
	// free(m_tx_cbs);
	// free(m_rx_cbs);
	// delete [] m_tx_cbs;
	// delete [] m_rx_cbs;
...
boolean Bcm54213Initialize (void)
{
	// probe:
...
const TMACAddress *Bcm54213GetMACAddress (void) 
{
	return &m_MACAddress;
}

boolean Bcm54213IsSendFrameAdvisable (void)
{
...
boolean Bcm54213SendFrame (const void *pBuffer, unsigned nLength)
{
	assert (pBuffer != 0);
	assert (nLength > 0);
	...
	if (0 != nic_mutex_lock())
	{
		LogWrite(FromBcm54213,USPI_LOG_ERROR,"%s: failed to lock mutex!", __func__);
	}
	// SpinLockAcquire(&m_TxSpinLock);
	...
	if (ring->free_bds < 2)				// is there room for this frame?
	{
		...
		if (0 != nic_mutex_unlock())
		{
			LogWrite(FromBcm54213,USPI_LOG_ERROR,"%s: failed to unlock mutex!", __func__);
		}
		// SpinLockRelease(&m_TxSpinLock);
		// m_TxSpinLock.Release ();

		return FALSE;
	}

	u8 *pTxBuffer = (u8 *) dma_alloc(DMA_PAGE_SIZE,DMA_ALIGNEMENT);
	// u8 *pTxBuffer = (u8 *) dma_alloc(ENET_MAX_MTU_SIZE * sizeof(u8),DMA_ALIGNEMENT);
	// u8 *pTxBuffer = (u8 *) malloc(ENET_MAX_MTU_SIZE * sizeof(u8));
	...
	// prepare for DMA
	// dma_man_cache_op(pTxBuffer, nLength, DMA_CACHE_OP_CLEAN_INVALIDATE);
	// int ret = genetBaseReg_cache_op((u32) (uintptr) pTxBuffer, nLength, DMA_CACHE_OP_CLEAN_INVALIDATE);
	// if(ret)
	// {
	// 	pr_err("CleanAndInvalidateDataCacheRange() failed.");
	// 	return 0;	
	// }
...
boolean Bcm54213ReceiveFrame (void *pBuffer, unsigned *pResultLength)
{
	assert (pBuffer != 0);
	...
	if (rxpkttoprocess > 0)
	{
		    ...
		    || !(dma_flag & DMA_SOP))
		{
			...
			dma_free(pRxBuffer,DMA_ALIGNEMENT);
			// free(pRxBuffer);
			// delete [] pRxBuffer;
...
boolean Bcm54213IsLinkUp (void)
{
	return m_link ? TRUE : FALSE;
}

TNetDeviceSpeed Bcm54213GetLinkSpeed (void)
{
	if (!m_link)
	{
...
boolean Bcm54213UpdatePHY (void)
{
...
void tx_ring16_int_enable(TGEnetTxRing *ring)
{
	intrl2_0_writel(UMAC_IRQ_TXDMA_DONE, INTRL2_CPU_MASK_CLEAR);
}

void tx_ring_int_enable(TGEnetTxRing *ring)
{
	intrl2_1_writel(1 << ring->index, INTRL2_CPU_MASK_CLEAR);
}

void rx_ring16_int_enable(TGEnetRxRing *ring)
{
...
int set_hw_addr(void)
{
	...
	MACAddressSet(&m_MACAddress,address);
	// m_MACAddress.Set (MACAddress.Address);

	TString MACString;
	String(&MACString);
	MACAddressFormat(&m_MACAddress,&MACString);
	// m_MACAddress.Format (&MACString);
	pr_debug("MAC address is %s", StringGet(&MACString));
...
void set_rx_mode(void)
{
	...
	// Broadcast
	TMACAddress Broadcast;
	MACAddressSetBroadcast(&Broadcast);
	// Broadcast.SetBroadcast();
	MACAddressCopyTo(&Broadcast,Buffer);
	// Broadcast.CopyTo(Buffer);
	set_mdf_addr(Buffer, &i, &mc);

	// my own address
	MACAddressCopyTo(&m_MACAddress,Buffer);
...
int init_dma(void)
{
	...
	m_rx_cbs = (TGEnetCB *) dma_alloc(DMA_PAGE_SIZE,DMA_ALIGNEMENT);
	// m_rx_cbs = (TGEnetCB *) dma_alloc(TOTAL_DESC * sizeof(TGEnetCB),DMA_ALIGNEMENT);
	// m_rx_cbs = (TGEnetCB *) malloc(TOTAL_DESC * sizeof(TGEnetCB));
	...
	m_tx_cbs = (TGEnetCB *) dma_alloc(DMA_PAGE_SIZE,DMA_ALIGNEMENT);
	// m_tx_cbs = (TGEnetCB *) dma_alloc(TOTAL_DESC * sizeof(TGEnetCB),DMA_ALIGNEMENT);
	// m_tx_cbs = (TGEnetCB *) malloc(TOTAL_DESC * sizeof(TGEnetCB));
...
void init_tx_ring(unsigned index, unsigned size,
...
struct TGEnetCB *get_txcb(TGEnetTxRing *ring)
{
...
int init_rx_ring(unsigned index, unsigned size,
				  unsigned start_ptr, unsigned end_ptr)
{
...
u8 *rx_refill(struct TGEnetCB *cb)
{
	// Allocate a new Rx DMA buffer
	u8 *buffer = (u8 *) dma_alloc(DMA_PAGE_SIZE,DMA_ALIGNEMENT);
	// u8 *buffer = (u8 *) dma_alloc(RX_BUF_LENGTH * sizeof(u8),DMA_ALIGNEMENT);
	// u8 *buffer = (u8 *) malloc(RX_BUF_LENGTH * sizeof(u8));
	// u8 *buffer = new u8[RX_BUF_LENGTH];
	if (!buffer)
		return 0;

	// prepare buffer for DMA
	// dma_man_cache_op(buffer, RX_BUF_LENGTH, DMA_CACHE_OP_CLEAN_INVALIDATE);
	// int ret = genetBaseReg_cache_op((u32) (uintptr) buffer, RX_BUF_LENGTH, DMA_CACHE_OP_CLEAN_INVALIDATE);
	// if(ret)
	// {
	// 	pr_err("CleanAndInvalidateDataCacheRange() failed.");
	// 	return 0;	
...
void dmadesc_set_addr(uintptr d, u8 *addr)
{
	...
	if(physAddr == 0)
	{
		LogWrite (FromBcm54213, USPI_LOG_ERROR, "Physical address conversion failed! buffer = %p", addr);
	}
...
void InterruptStub0 (void *pParam)
{
	InterruptHandler0();
...
void InterruptStub1 (void *pParam)
{
	InterruptHandler1();
	// CBcm54213Device *pThis = (CBcm54213Device *) pParam;
	// assert (pThis != 0);
...
void mdio_write(int reg, u16 val)
{
